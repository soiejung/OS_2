...
void add_proc_list(char path[PATH_LEN], bool isPPS, bool aOption, bool uOption, bool xOption, unsigned long cpuTimeTable[PID_MAX])
{
	...
	long double cpu = 0;
	bool update = false;
	if(!isPPS && cpuTimeTable != NULL){			//mytop일 경우
	...
	if(isPPS){		//myps인 경우에만 세부 state 추가
	...
	if(!isPPS || (!aOption && !uOption && !xOption))	//mytop이거나 myps에서 옵션이 없을 경우
	...
	if(!strlen(proc.command))				//cmdline에서 읽은 문자열 길이 0일 경우
		sprintf(proc.command, "[%s]", proc.cmd);	// [cmd]로 채우기
	fclose(cmdLineFp);
